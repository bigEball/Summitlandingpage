...
const { render, SEO_ROUTES, absoluteUrl } = await import(
  path.join(rootDir, 'dist-ssr', 'entry-prerender.js')
);
...
// page would then be built on top of the landing page's markup. Cheaper to
// refuse than to debug later.
...
/**
 * Every route on this site is in `SEO_ROUTES`, so every URL now has a real file
 * and there is nothing left for an SPA fallback to answer. When the marketing
 * pages shared a build with the application, this is where an extra empty shell
 * was written for the sign-in and the app screens; here a URL that matched no
 * file should simply 404, which is why `netlify.toml` has no catch-all rewrite.
 */
if (written !== SEO_ROUTES.length) {
  throw new Error(
    `prerender: wrote ${written} of ${SEO_ROUTES.length} routes — every route in ` +
      'SEO_ROUTES must produce a file, because nothing serves the ones that do not.',
  );
}
